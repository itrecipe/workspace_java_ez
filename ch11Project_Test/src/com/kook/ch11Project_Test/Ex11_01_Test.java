package com.kook.ch11Project_Test;

import java.util.*;

public class Ex11_01_Test {

	public static void main(String[] args) {
		//ArrayList객체는 생성자를 이용
		//ArrayList list1 = new ArrayList(10); //원시적 작성방법
		//10은 초기 용량
		//ArrayList클래스는 지네릭 클래스로 사용 안하면 원시타입이다.
		ArrayList<Integer> list1 = new ArrayList<Integer>(10); //<- 요즘은 이렇게 작성하는것이 정상
		//List인터페이스 계열로 배열기반으로서 순서를 유지하고 중복을 허용하고 저장 메모리가 연속되어 사용
		//ArrayList객체에 요소값을 추가할시 add메서드 사용
		list1.add(5); //5는 기본형이나 자동박싱(오토박싱)에 의해 Integer타입으로 변환
		list1.add(4);
		list1.add(2);
		list1.add(0);
		list1.add(1);
		...
		*/
		...
		System.out.println("list1.containsAll(list2) : " 
							+ list1.containsAll(list2)); //list1은 list2 모두 포함하는가?
		
		list2.add(6);
		list2.add(7);
		//add(값)은 마지막 요소뒤에 추가
		list2.add(3, 8); //이 방법은 속도가 늦어지므로 좋지 않은 방식
		/* add(색인번호, 값)은 색인번호의 값을 추가
		   뒤의 원소는 뒤로 밀림
		*/
		print(list1, list2);
		
		list2.set(3, 44); //set은 색인번호 3번 변경
		print(list1, list2);
		
		/* list1에서 list2와 겹치는 부분만 남기고 나머진 삭제함(retainAll메서드)
